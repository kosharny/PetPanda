...
struct QuizView: View {
    ...
    init(quizId: String, repository: QuizRepository, onBackTap: @escaping () -> Void, onReady: @escaping () -> Void) {
        _vm = StateObject(wrappedValue: QuizViewModel(quizId: quizId, repository: repository))
    ...
    private func renderOverviewMode(quiz: Quiz) -> some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 20) {
                ...
                HStack {
                    CotegoryButton(title: quiz.categoryId, onTap: {})
                    CotegoryButton(title: "\(quiz.questions.count) Questions", onTap: {})
                    CotegoryButton(title: "\(quiz.duration) min", onTap: {})
                }
                .padding(.horizontal)
